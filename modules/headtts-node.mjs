...
const maxRestQueue = parseInt(getOptionValue('max-rest-queue')) || 32;
const restResponseTimeoutMs = parseInt(getOptionValue('rest-timeout')) || 120000;
let websocketId = 0;
let restId = 0;
      ...
      req.on('end', () => {
        ...
        try {
          ...
          if ( !input.trim() ) {
            res.statusCode = 400;
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ error: "Field 'input' must be a non-empty string." }));
            return;
          }
          if ( queue.length >= maxRestQueue ) {
            res.statusCode = 503;
            res.setHeader('Content-Type', 'application/json');
            res.end(JSON.stringify({ error: "Server is busy. Please retry shortly." }));
            return;
          }
          ...
          const cleanup = () => mapRest.delete(id);
          req.on('aborted', cleanup);
          res.on('close', cleanup);
          res.setTimeout(restResponseTimeoutMs, () => {
            cleanup();
            if ( !res.writableEnded ) {
              res.statusCode = 504;
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ error: "Synthesis timed out." }));
            }
          });
